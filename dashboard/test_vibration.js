...
const ids=["vibExport","vibBaseCount","vibRunCount","vibValCount","vibBase","vibRun","vibStop","vibAnalyze","vibValidate","vibReset","vibStatus","vibChart","vibResults"];
...
function sample(i,active){
  ...
  const a=active?1.8*Math.sin(2*Math.PI*120*i/800):.01*noise;
  const g=active?120*Math.sin(2*Math.PI*180*i/800):.1*noise;
  return{acc:[Math.round(a*2048),0,2048],gyr:[Math.round(g*16.4),0,0],seq:i&65535};
}
elements.vibBase.onclick();for(let i=0;i<4000;i++)context.vibrationAddSample(sample(i,false));elements.vibStop.onclick();
elements.vibRun.onclick();for(let i=0;i<4000;i++)context.vibrationAddSample(sample(i,true));elements.vibStop.onclick();
assert.strictEqual(elements.vibAnalyze.disabled,false);elements.vibAnalyze.onclick();
setTimeout(()=>{
  assert.match(elements.vibResults.innerHTML,/120\.3 Hz/);
  assert.match(elements.vibResults.innerHTML,/179\.7 Hz/);
  assert.strictEqual(elements.vibValidate.disabled,false);
  elements.vibValidate.onclick();for(let i=0;i<3200;i++)context.vibrationAddSample(sample(i,true));elements.vibStop.onclick();
